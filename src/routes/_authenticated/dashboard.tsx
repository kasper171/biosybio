  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        overlay_noise_enabled: profile.overlay_noise_enabled === true,
        overlay_noise_opacity: normalizeOverlayNoiseOpacity(profile.overlay_noise_opacity),
      })
      .eq("id", profile.id);
    if (error) {
      setSaving(false);
        ...
        <div className="biosy-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {openPanel === "overlays" && <OverlaysPanel profile={profile} update={update} />}
        ...
        <SliderField
          ...
          display={
            getRoleBadgeGapPx(profile) === 0
              ? t("dashboard.perfil.roleBadges.spacingTouching")
              : `${getRoleBadgeGapPx(profile)}px`
          }
        />
        <p className="text-[11px] leading-relaxed text-white/40">
          {t("dashboard.perfil.roleBadges.spacingHint")}
        </p>
        <ToggleField
