-- Role badge placement on profile card.
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS role_badges_placement text NOT NULL DEFAULT 'below_name';

ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_role_badges_placement_check;

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_role_badges_placement_check
  CHECK (role_badges_placement IN ('below_name', 'inline_name', 'below_socials'));

COMMENT ON COLUMN public.profiles.role_badges_placement IS
  'Role badge position: below_name | inline_name | below_socials';
